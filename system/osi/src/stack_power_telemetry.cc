...
constexpr std::string_view kPowerTelemetryEnabledProperty = "bluetooth.powertelemetry.enabled";
...
struct power_telemetry::PowerTelemetryImpl {
  PowerTelemetryImpl() {
    ...
    log_per_channel_ = osi_property_get_bool(std::string(kLogPerChannelProperty).c_str(), false);
  ...
  LogDataContainer& GetCurrentLogDataContainer() { return log_data_containers_[idx_containers]; }
...
void power_telemetry::PowerTelemetryImpl::RecordLogDataContainer() {
  ...
  log::info(
          ...
          "hciEvt: {}, bleScan: {}",
          ldc.scan_details.count, ldc.inq_scan_details.count, ldc.acl_pkt_ds.tx.pkt_count,
          ldc.acl_pkt_ds.rx.pkt_count, ldc.hci_cmd_evt_ds.tx.pkt_count,
          ldc.hci_cmd_evt_ds.rx.pkt_count, ldc.le_scan_details.count);
...
void power_telemetry::PowerTelemetry::LogSniffStarted(uint16_t handle, const RawAddress& bd_addr) {
  if (!power_telemerty_enabled_) {
    return;
  }
  ...
  sniff_data.active_duration_ts += current_timestamp - sniff_data.last_mode_change_ts;
  sniff_data.last_mode_change_ts = get_current_time();
  ldc.sniff_activity_map[handle] = sniff_data;
...
void power_telemetry::PowerTelemetry::LogSniffStopped(uint16_t handle, const RawAddress& bd_addr) {
  if (!power_telemerty_enabled_) {
    return;
  }
  ...
  sniff_data.sniff_duration_ts += current_timestamp - sniff_data.last_mode_change_ts;
  sniff_data.last_mode_change_ts = get_current_time();
  ldc.sniff_activity_map[handle] = sniff_data;
...
void power_telemetry::PowerTelemetry::LogChannelDisconnected(uint16_t psm, int32_t src_id,
                                                             int32_t dst_id,
                                                             const RawAddress& bd_addr) {
  if (!power_telemerty_enabled_) {
    return;
...
void power_telemetry::PowerTelemetry::LogTxBytes(uint16_t psm, int32_t src_id, int32_t dst_id,
  ...
  if (pimpl_->log_per_channel_ == true) {
    ...
    for (auto& channel_details : ldc.channel_map[bd_addr]) {
      if (channel_details.src.cid == src_id && channel_details.dst.cid == dst_id &&
...
void power_telemetry::PowerTelemetry::LogRxBytes(uint16_t psm, int32_t src_id, int32_t dst_id,
                                                 const RawAddress& bd_addr, int32_t num_bytes) {
  if (!power_telemerty_enabled_) {
    return;
  }
...
void power_telemetry::PowerTelemetry::Dumpsys(int32_t fd) {
  ...
  for (auto&& ldc : pimpl_->log_data_containers_) {
    if ((ldc.acl_pkt_ds.tx.byte_count == 0) && (ldc.acl_pkt_ds.rx.byte_count == 0)) {
      continue;
    }
  ...
  dprintf(fd, "%-22s %-22s %-14s %-14s\n", "StartTimeStamp", "EndTimeStamp", "HCI Commands",
          "HCI Events");
  for (auto&& ldc : pimpl_->log_data_containers_) {
    if ((ldc.hci_cmd_evt_ds.tx.pkt_count == 0) && (ldc.hci_cmd_evt_ds.rx.pkt_count == 0)) {
      continue;
    }
    dprintf(fd, "%-22s %-22s %-14d %-14d\n", GetTimeString(ldc.lifetime.begin).c_str(),
            GetTimeString(ldc.lifetime.end).c_str(), ldc.hci_cmd_evt_ds.tx.pkt_count,
            ldc.hci_cmd_evt_ds.rx.pkt_count);
  }
  dprintf(fd, "\nBLE Scan Details:\n");
  ...
  dprintf(fd, "%-19s %-7s %-7s %-7s %-8s %-22s", "RemoteAddress", "Type", "SrcId", "DstId", "PSM",
          "ConnectedTimeStamp");
  dprintf(fd, " %-22s %-14s ", "DisconnectedTimeStamp", "State");
  if (pimpl_->log_per_channel_ == true) {
    dprintf(fd, " %-10s %-10s %-22s %-22s", "TxBytes", "RxBytes", "LastTxTimeStamp",
            "LastRxTimeStamp");
  }
  dprintf(fd, "\n");
  for (auto&& ldc : pimpl_->log_data_containers_) {
  ...
  dprintf(fd, "%-8s %-19s %-19s %-24s %-19s %-24s\n", "Handle", "BDADDR", "ActiveModeCount",
          "ActiveModeDuration(sec)", "SniffModeCount", "SniffModeDuration(sec)");
  for (auto&& ldc : pimpl_->log_data_containers_) {
