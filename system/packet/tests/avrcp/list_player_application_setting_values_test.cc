...
using ListPlayerApplicationSettingAttributeValuesRspTestPacket = TestPacketType<Packet>;
...
TEST(ListPlayerApplicationSettingValuesRequestPacketTest, invalidTest) {
  std::vector<uint8_t> packet_copy = list_player_application_setting_attribute_values_request;
  packet_copy.push_back(0x00);
  auto test_packet = ListPlayerApplicationSettingValuesRequestTestPacket::Make(packet_copy);
  ASSERT_FALSE(test_packet->IsValid());

  std::vector<uint8_t> short_packet = {
          0, 1, 2, 3, 4, 5, 6,
  };
  test_packet = ListPlayerApplicationSettingValuesRequestTestPacket::Make(short_packet);
  ASSERT_FALSE(test_packet->IsValid());
}

TEST(ListPlayerApplicationSettingValuesResponseBuilderTest, builderTest) {
  std::vector<uint8_t> vals = {0x01, 0x02, 0x03, 0x04};  // All possible repeat vals
  auto builder = ListPlayerApplicationSettingValuesResponseBuilder::MakeBuilder(vals);

  ASSERT_EQ(builder->size(), list_player_application_setting_attribute_values_response.size());

  auto test_packet = ListPlayerApplicationSettingAttributeValuesRspTestPacket::Make();
  builder->Serialize(test_packet);
  ASSERT_EQ(test_packet->GetData(), list_player_application_setting_attribute_values_response);
}
