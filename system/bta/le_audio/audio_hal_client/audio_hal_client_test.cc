...
public:
  ...
  MOCK_METHOD((void), SetPcmParameters, (const LeAudioClientInterface::PcmParameters& params),
              (override));
...
LeAudioClientInterface::Sink* LeAudioClientInterface::GetSink(
        ...
        bool is_broadcasting_session_type) {
  return interface_mock->GetSink(stream_cb, message_loop, is_broadcasting_session_type);
}
...
class MockLeAudioClientAudioSinkEventReceiver : public LeAudioSourceAudioHalClient::Callbacks {
public:
...
class MockAudioHalClientEventReceiver : public LeAudioSinkAudioHalClient::Callbacks {
public:
  MOCK_METHOD((void), OnAudioSuspend, (), (override));
  MOCK_METHOD((void), OnAudioResume, (), (override));
  MOCK_METHOD((void), OnAudioMetadataUpdate,
...
protected:
  ...
  void TearDown(void) override {
     ...
     */
    if (is_source_audio_hal_acquired == true) {
      ReleaseLeAudioSinkHalClient();
    }
    if (is_sink_audio_hal_acquired == true) {
      ReleaseLeAudioSourceHalClient();
    }
...
TEST_F(LeAudioClientAudioTest, testLeAudioClientAudioSinkStartStop) {
  ...
  ASSERT_TRUE(AcquireLeAudioSinkHalClient());
  ASSERT_TRUE(audio_sink_instance_->Start(default_codec_conf, &mock_hal_source_event_receiver_));

  ASSERT_EQ(params.channels_count, bluetooth::audio::le_audio::kChannelNumberMono);
  ASSERT_EQ(params.sample_rate, bluetooth::audio::le_audio::kSampleRate44100);
  ASSERT_EQ(params.bits_per_sample, bluetooth::audio::le_audio::kBitsPerSample24);
...
TEST_F(LeAudioClientAudioTest, testAudioHalClientStartStop) {
  ...
  ASSERT_TRUE(AcquireLeAudioSourceHalClient());
  ASSERT_TRUE(audio_source_instance_->Start(default_codec_conf, &mock_hal_sink_event_receiver_));
...
TEST_F(LeAudioClientAudioTest, testLeAudioClientAudioSinkSendData) {
  ...
  ON_CALL(mock_hal_interface_audio_source_, Write(_, _))
          .WillByDefault(DoAll(SaveArg<0>(&exp_p), SaveArg<1>(&exp_len), ReturnPointee(&exp_len)));

  ASSERT_EQ(audio_sink_instance_->SendData(input_buf, sizeof(input_buf)), sizeof(input_buf));
