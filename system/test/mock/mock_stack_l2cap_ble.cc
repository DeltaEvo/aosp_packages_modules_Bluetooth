...
struct l2c_link_process_ble_num_bufs l2c_link_process_ble_num_bufs;
...
bool L2CA_UpdateBleConnParams(const RawAddress& rem_bda, uint16_t min_int, uint16_t max_int,
  ...
  return test::mock::stack_l2cap_ble::L2CA_UpdateBleConnParams(rem_bda, min_int, max_int, latency,
                                                               timeout, min_ce_len, max_ce_len);
}
void L2CA_LockBleConnParamsForServiceDiscovery(const RawAddress& rem_bda, bool enable) {
  inc_func_call_count(__func__);
  return test::mock::stack_l2cap_ble::L2CA_LockBleConnParamsForServiceDiscovery(rem_bda, enable);
}
...
void l2cble_process_conn_update_evt(uint16_t handle, uint8_t status, uint16_t interval,
  ...
  inc_func_call_count(__func__);
  test::mock::stack_l2cap_ble::l2cble_process_conn_update_evt(handle, status, interval, latency,
                                                              timeout);
}
...
void l2c_link_process_ble_num_bufs(uint16_t num_lm_ble_bufs) {
  inc_func_call_count(__func__);
  test::mock::stack_l2cap_ble::l2c_link_process_ble_num_bufs(num_lm_ble_bufs);
}
...
void l2cble_process_rc_param_request_evt(uint16_t handle, uint16_t int_min, uint16_t int_max,
                                         uint16_t latency, uint16_t timeout) {
  inc_func_call_count(__func__);
  test::mock::stack_l2cap_ble::l2cble_process_rc_param_request_evt(handle, int_min, int_max,
                                                                   latency, timeout);
}
...
void l2cble_send_flow_control_credit(tL2C_CCB* p_ccb, uint16_t credit_value) {
  inc_func_call_count(__func__);
  test::mock::stack_l2cap_ble::l2cble_send_flow_control_credit(p_ccb, credit_value);
}
...
tL2CAP_LE_RESULT_CODE l2ble_sec_access_req(const RawAddress& bd_addr, uint16_t psm,
                                           ...
                                           void* p_ref_data) {
  inc_func_call_count(__func__);
  return test::mock::stack_l2cap_ble::l2ble_sec_access_req(bd_addr, psm, is_originator, p_callback,
                                                           p_ref_data);
}
void L2CA_AdjustConnectionIntervals(uint16_t* min_interval, uint16_t* max_interval,
                                    uint16_t floor_interval) {
  inc_func_call_count(__func__);
  test::mock::stack_l2cap_ble::L2CA_AdjustConnectionIntervals(min_interval, max_interval,
                                                              floor_interval);
}
...
bool L2CA_SubrateRequest(const RawAddress& rem_bda, uint16_t subrate_min, uint16_t subrate_max,
  ...
  inc_func_call_count(__func__);
  return test::mock::stack_l2cap_ble::L2CA_SubrateRequest(rem_bda, subrate_min, subrate_max,
