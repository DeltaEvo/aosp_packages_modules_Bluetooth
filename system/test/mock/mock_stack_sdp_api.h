...
struct SDP_FindProfileVersionInRec {
  std::function<bool(const tSDP_DISC_REC* p_rec, uint16_t profile_uuid, uint16_t* p_version)> body{
          [](const tSDP_DISC_REC* /* p_rec */, uint16_t /* profile_uuid */,
             uint16_t* /* p_version */) { return false; }};
  bool operator()(const tSDP_DISC_REC* p_rec, uint16_t profile_uuid, uint16_t* p_version) {
...
struct SDP_FindProtocolListElemInRec {
  std::function<bool(const tSDP_DISC_REC* p_rec, uint16_t layer_uuid, tSDP_PROTOCOL_ELEM* p_elem)>
          body{[](const tSDP_DISC_REC* /* p_rec */, uint16_t /* layer_uuid */,
                  tSDP_PROTOCOL_ELEM* /* p_elem */) { return false; }};
  bool operator()(const tSDP_DISC_REC* p_rec, uint16_t layer_uuid, tSDP_PROTOCOL_ELEM* p_elem) {
...
struct SDP_FindServiceUUIDInRec {
  std::function<bool(const tSDP_DISC_REC* p_rec, bluetooth::Uuid* p_uuid)> body{
          [](const tSDP_DISC_REC* /* p_rec */, bluetooth::Uuid* /* p_uuid */) { return false; }};
...
struct SDP_ServiceSearchAttributeRequest {
  std::function<bool(const RawAddress& p_bd_addr, tSDP_DISCOVERY_DB* p_db, tSDP_DISC_CMPL_CB* p_cb)>
          body{[](const RawAddress& /* p_bd_addr */, tSDP_DISCOVERY_DB* /* p_db */,
                  tSDP_DISC_CMPL_CB* /* p_cb */) { return false; }};
  bool operator()(const RawAddress& p_bd_addr, tSDP_DISCOVERY_DB* p_db, tSDP_DISC_CMPL_CB* p_cb) {
    return body(p_bd_addr, p_db, p_cb);
  }
};
extern struct SDP_ServiceSearchAttributeRequest SDP_ServiceSearchAttributeRequest;
...
extern struct SDP_ServiceSearchAttributeRequest2 SDP_ServiceSearchAttributeRequest2;
...
struct SDP_FindAttributeInRec {
  std::function<tSDP_DISC_ATTR*(const tSDP_DISC_REC* p_rec, uint16_t attr_id)> body{
...
struct SDP_FindServiceInDb {
  std::function<tSDP_DISC_REC*(const tSDP_DISCOVERY_DB* p_db, uint16_t service_uuid,
...
struct SDP_FindServiceInDb_128bit {
  std::function<tSDP_DISC_REC*(const tSDP_DISCOVERY_DB* p_db, tSDP_DISC_REC* p_start_rec)> body{
          [](const tSDP_DISCOVERY_DB* /* p_db */, tSDP_DISC_REC* /* p_start_rec */) {
            return nullptr;
          }};
  tSDP_DISC_REC* operator()(const tSDP_DISCOVERY_DB* p_db, tSDP_DISC_REC* p_start_rec) {
...
struct SDP_FindServiceUUIDInDb {
  std::function<tSDP_DISC_REC*(const tSDP_DISCOVERY_DB* p_db, const bluetooth::Uuid& uuid,
                               tSDP_DISC_REC* p_start_rec)>
          body{[](const tSDP_DISCOVERY_DB* /* p_db */, const bluetooth::Uuid& /* uuid */,
                  tSDP_DISC_REC* /* p_start_rec */) { return nullptr; }};
  tSDP_DISC_REC* operator()(const tSDP_DISCOVERY_DB* p_db, const bluetooth::Uuid& uuid,
                            tSDP_DISC_REC* p_start_rec) {
