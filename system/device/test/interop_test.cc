...
protected:
  virtual void SetUp() override {
    ...
    ASSERT_EQ(fsync(fileno(fp)), 0) << "Associated ERRNO error is: " << strerror(errno);
...
TEST_F(InteropTest, test_lookup_miss) {
  ...
  EXPECT_FALSE(interop_match_addr(INTEROP_KEYBOARD_REQUIRES_FIXED_PIN, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_2MBPS_LINK_ONLY, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_HID_PREF_CONN_SUP_TIMEOUT_3S, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_GATTC_NO_SERVICE_CHANGED_IND, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_DISABLE_SDP_AFTER_PAIRING, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_DISABLE_AUTH_FOR_HID_POINTING, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_REMOVE_HID_DIG_DESCRIPTOR, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_DISABLE_SNIFF_DURING_SCO, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_INCREASE_AG_CONN_TIMEOUT, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_DISABLE_LE_CONN_PREFERRED_PARAMS, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_DISABLE_AAC_CODEC, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_DISABLE_AAC_VBR_CODEC, &test_address));
  ...
  EXPECT_FALSE(interop_match_addr(INTEROP_DISABLE_ROLE_SWITCH_POLICY, &test_address));
  ...
  EXPECT_FALSE(interop_match_addr(INTEROP_UPDATE_HID_SSR_MAX_LAT, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_DISABLE_AVDTP_RECONFIGURE, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_DISABLE_HF_INDICATOR, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_DISABLE_LE_CONN_UPDATES, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_DELAY_SCO_FOR_MT_CALL, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_DISABLE_CODEC_NEGOTIATION, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_DISABLE_PLAYER_APPLICATION_SETTING_CMDS, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_ENABLE_AAC_CODEC, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_DISABLE_CONNECTION_AFTER_COLLISION, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_AVRCP_BROWSE_OPEN_CHANNEL_COLLISION, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_ADV_PBAP_VER_1_2, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_DISABLE_PCE_SDP_AFTER_PAIRING, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_DISABLE_SNIFF_LINK_DURING_SCO, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_DISABLE_SNIFF_DURING_CALL, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_HID_HOST_LIMIT_SNIFF_INTERVAL, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_DISABLE_REFRESH_ACCEPT_SIG_TIMER, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_SKIP_INCOMING_STATE, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_NOT_UPDATE_AVRCP_PAUSED_TO_REMOTE, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_PHONE_POLICY_INCREASED_DELAY_CONNECT_OTHER_PROFILES,
                                  &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_PHONE_POLICY_REDUCED_DELAY_CONNECT_OTHER_PROFILES,
                                  &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_HFP_FAKE_INCOMING_CALL_INDICATOR, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_HFP_SEND_CALL_INDICATORS_BACK_TO_BACK, &test_address));
  EXPECT_FALSE(
          interop_match_addr(INTEROP_SETUP_SCO_WITH_NO_DELAY_AFTER_SLC_DURING_CALL, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_ENABLE_PREFERRED_CONN_PARAMETER, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_RETRY_SCO_AFTER_REMOTE_REJECT_SCO, &test_address));
  EXPECT_FALSE(interop_match_addr(INTEROP_DELAY_SCO_FOR_MO_CALL, &test_address));
  ...
  EXPECT_FALSE(interop_match_addr(INTEROP_DISABLE_ROLE_SWITCH_DURING_CONNECTION, &test_address));
...
TEST_F(InteropTest, test_name_hit) {
  ...
  EXPECT_TRUE(interop_match_name(INTEROP_GATTC_NO_SERVICE_CHANGED_IND, "MiMouse"));
  ...
  EXPECT_TRUE(interop_match_name(INTEROP_HID_HOST_LIMIT_SNIFF_INTERVAL, "Joy-Con"));
  EXPECT_TRUE(interop_match_name(INTEROP_DISABLE_REFRESH_ACCEPT_SIG_TIMER, "HB20"));
  EXPECT_TRUE(interop_match_name(INTEROP_NOT_UPDATE_AVRCP_PAUSED_TO_REMOTE, "Audi"));
  EXPECT_TRUE(
          interop_match_name(INTEROP_SETUP_SCO_WITH_NO_DELAY_AFTER_SLC_DURING_CALL, "Geely_BT"));
  EXPECT_TRUE(
...
TEST_F(InteropTest, test_name_miss) {
  ...
  EXPECT_FALSE(interop_match_name(INTEROP_DISABLE_LE_SECURE_CONNECTIONS, "TEST"));
  ...
  EXPECT_FALSE(interop_match_name(INTEROP_HID_PREF_CONN_SUP_TIMEOUT_3S, "TEST"));
  EXPECT_FALSE(interop_match_name(INTEROP_GATTC_NO_SERVICE_CHANGED_IND, "TEST"));
  EXPECT_FALSE(interop_match_name(INTEROP_DISABLE_SDP_AFTER_PAIRING, "TEST"));
  EXPECT_FALSE(interop_match_name(INTEROP_DISABLE_AUTH_FOR_HID_POINTING, "TEST"));
  ...
  EXPECT_FALSE(interop_match_name(INTEROP_DISABLE_LE_CONN_PREFERRED_PARAMS, "TEST"));
  ...
  EXPECT_FALSE(interop_match_name(INTEROP_DISABLE_PLAYER_APPLICATION_SETTING_CMDS, "TEST"));
  EXPECT_FALSE(interop_match_name(INTEROP_ENABLE_AAC_CODEC, "TEST"));
  EXPECT_FALSE(interop_match_name(INTEROP_DISABLE_CONNECTION_AFTER_COLLISION, "TEST"));
  EXPECT_FALSE(interop_match_name(INTEROP_AVRCP_BROWSE_OPEN_CHANNEL_COLLISION, "TEST"));
  EXPECT_FALSE(interop_match_name(INTEROP_ADV_PBAP_VER_1_2, "TEST"));
  EXPECT_FALSE(interop_match_name(INTEROP_DISABLE_PCE_SDP_AFTER_PAIRING, "TEST"));
  EXPECT_FALSE(interop_match_name(INTEROP_DISABLE_SNIFF_LINK_DURING_SCO, "TEST"));
  EXPECT_FALSE(interop_match_name(INTEROP_DISABLE_SNIFF_DURING_CALL, "TEST"));
  EXPECT_FALSE(interop_match_name(INTEROP_HID_HOST_LIMIT_SNIFF_INTERVAL, "TEST"));
  EXPECT_FALSE(interop_match_name(INTEROP_DISABLE_REFRESH_ACCEPT_SIG_TIMER, "TEST"));
  EXPECT_FALSE(interop_match_name(INTEROP_SKIP_INCOMING_STATE, "TEST"));
  EXPECT_FALSE(interop_match_name(INTEROP_NOT_UPDATE_AVRCP_PAUSED_TO_REMOTE, "TEST"));
  EXPECT_FALSE(
          interop_match_name(INTEROP_PHONE_POLICY_INCREASED_DELAY_CONNECT_OTHER_PROFILES, "TEST"));
  EXPECT_FALSE(
          interop_match_name(INTEROP_PHONE_POLICY_REDUCED_DELAY_CONNECT_OTHER_PROFILES, "TEST"));
  EXPECT_FALSE(interop_match_name(INTEROP_HFP_FAKE_INCOMING_CALL_INDICATOR, "TEST"));
  EXPECT_FALSE(interop_match_name(INTEROP_HFP_SEND_CALL_INDICATORS_BACK_TO_BACK, "TEST"));
  EXPECT_FALSE(interop_match_name(INTEROP_SETUP_SCO_WITH_NO_DELAY_AFTER_SLC_DURING_CALL, "TEST"));
  EXPECT_FALSE(interop_match_name(INTEROP_ENABLE_PREFERRED_CONN_PARAMETER, "TEST"));
  EXPECT_FALSE(interop_match_name(INTEROP_RETRY_SCO_AFTER_REMOTE_REJECT_SCO, "TEST"));
  ...
  EXPECT_FALSE(interop_match_name(INTEROP_DISABLE_ROLE_SWITCH_DURING_CONNECTION, "TEST"));
...
TEST_F(InteropTest, test_vndr_prdt_hit) {
  ...
  EXPECT_TRUE(interop_database_match_vndr_prdt(INTEROP_REMOVE_HID_DIG_DESCRIPTOR, vendor_id,
                                               product_id));

  vendor_id = 0x05ac;
  product_id = 0x0255;

  EXPECT_TRUE(interop_database_match_vndr_prdt(INTEROP_CHANGE_HID_VID_PID, vendor_id, product_id));
...
TEST_F(InteropTest, test_vndr_prdt_miss) {
  ...
  EXPECT_FALSE(interop_database_match_vndr_prdt(INTEROP_REMOVE_HID_DIG_DESCRIPTOR, vendor_id,
                                                product_id));
...
TEST_F(InteropTest, test_manufacturer_hit) {
  ...
  EXPECT_TRUE(interop_database_match_manufacturer(INTEROP_DISABLE_SNIFF_DURING_SCO, manufacturer));
...
TEST_F(InteropTest, test_manufacturer_miss) {
  ...
  EXPECT_FALSE(interop_database_match_manufacturer(INTEROP_DISABLE_SNIFF_DURING_SCO, manufacturer));
...
TEST_F(InteropTest, test_dynamic_addr) {
  ...
  interop_database_remove_addr(INTEROP_DISABLE_LE_SECURE_CONNECTIONS, &test_address);
  EXPECT_FALSE(interop_match_addr(INTEROP_DISABLE_LE_SECURE_CONNECTIONS, &test_address));
...
TEST_F(InteropTest, test_dynamic_name) {
  ...
  EXPECT_TRUE(interop_match_name(INTEROP_DISABLE_LE_SECURE_CONNECTIONS, "TEST"));
...
TEST_F(InteropTest, test_dynamic_vndr_prdt) {
  ...
  interop_database_add_vndr_prdt(INTEROP_REMOVE_HID_DIG_DESCRIPTOR, vendor_id, product_id);
  EXPECT_TRUE(interop_database_match_vndr_prdt(INTEROP_REMOVE_HID_DIG_DESCRIPTOR, vendor_id,
                                               product_id));

  interop_database_remove_vndr_prdt(INTEROP_REMOVE_HID_DIG_DESCRIPTOR, vendor_id, product_id);

  EXPECT_FALSE(interop_database_match_vndr_prdt(INTEROP_CHANGE_HID_VID_PID, vendor_id, product_id));
...
TEST_F(InteropTest, test_dynamic_manufacturer) {
  ...
  interop_database_add_manufacturer(INTEROP_DISABLE_SNIFF_DURING_SCO, manufacturer);

  EXPECT_TRUE(interop_database_match_manufacturer(INTEROP_DISABLE_SNIFF_DURING_SCO, manufacturer));

  interop_database_remove_manufacturer(INTEROP_DISABLE_SNIFF_DURING_SCO, manufacturer);
...
TEST_F(InteropTest, test_dynamic_addr_get_lmp_version) {
  ...
  EXPECT_FALSE(interop_database_match_addr_get_lmp_ver(
          INTEROP_DISABLE_SNIFF_DURING_SCO, &test_address, &lmp_version, &lmp_sub_version));
  interop_database_add_addr_lmp_version(INTEROP_DISABLE_SNIFF_DURING_SCO, &test_address, 0xFF,
                                        0xFFFF);
  ...
  interop_database_remove_addr_lmp_version(INTEROP_DISABLE_SNIFF_DURING_SCO, &test_address, 0xFF,
                                           0xFFFF);
...
TEST_F(InteropTest, test_dynamic_did_version) {
  ...
  EXPECT_TRUE(interop_database_match_version(INTEROP_DISABLE_SNIFF_DURING_SCO, did_version));

  interop_database_remove_version(INTEROP_DISABLE_SNIFF_DURING_SCO, did_version);
