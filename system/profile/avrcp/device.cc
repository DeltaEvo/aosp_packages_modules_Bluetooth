...
void Device::RegisterInterfaces(MediaInterface* media_interface, A2dpInterface* a2dp_interface,
                                VolumeInterface* volume_interface,
                                PlayerSettingsInterface* player_settings_interface) {
  log::assert_that(media_interface != nullptr, "assert failed: media_interface != nullptr");
  log::assert_that(a2dp_interface != nullptr, "assert failed: a2dp_interface != nullptr");
...
bool Device::IsActive() const { return address_ == a2dp_interface_->active_peer(); }
  ...
  if (pkt->GetCType() >= CType::ACCEPTED) {
    switch (pkt->GetCommandPdu()) {
      ...
      case CommandPdu::REGISTER_NOTIFICATION: {
        auto register_notification = Packet::Specialize<RegisterNotificationResponse>(pkt);
  ...
  switch (pkt->GetCommandPdu()) {
    ...
    case CommandPdu::GET_PLAY_STATUS: {
      media_interface_->GetPlayStatus(
              base::Bind(&Device::GetPlayStatusResponse, weak_ptr_factory_.GetWeakPtr(), label));
    } break;
    ...
    case CommandPdu::LIST_PLAYER_APPLICATION_SETTING_VALUES: {
      ...
      player_settings_interface_->ListPlayerSettingValues(
              attribute, base::Bind(&Device::ListPlayerApplicationSettingValuesResponse,
                                    weak_ptr_factory_.GetWeakPtr(), label));
    } break;
    ...
    case CommandPdu::GET_CURRENT_PLAYER_APPLICATION_SETTING_VALUE: {
      ...
      auto get_current_player_setting_value_request =
              Packet::Specialize<GetCurrentPlayerApplicationSettingValueRequest>(pkt);
    ...
    case CommandPdu::SET_PLAYER_APPLICATION_SETTING_VALUE: {
      ...
      std::vector<uint8_t> values = set_player_setting_value_request->GetRequestedValues();
...
void Device::HandleGetCapabilities(uint8_t label,
  ...
  switch (pkt->GetCapabilityRequested()) {
    case Capability::COMPANY_ID: {
      auto response = GetCapabilitiesResponseBuilder::MakeCompanyIdBuilder(0x001958);
      response->AddCompanyId(0x002345);
      send_message_cb_.Run(label, false, std::move(response));
    } break;

    case Capability::EVENTS_SUPPORTED: {
      auto response = GetCapabilitiesResponseBuilder::MakeEventsSupportedBuilder(
    ...
    default: {
      log::warn("{}: Unhandled Capability: {}", address_, pkt->GetCapabilityRequested());
      auto response =
...
void Device::HandleNotification(uint8_t label,
  ...
  switch (pkt->GetEventRegistered()) {
    ...
    case Event::AVAILABLE_PLAYERS_CHANGED: {
      ...
      auto response = RegisterNotificationResponseBuilder::MakeAvailablePlayersBuilder(true);
      send_message(label, false, std::move(response));
    } break;
    ...
    case Event::UIDS_CHANGED: {
      ...
      auto response = RegisterNotificationResponseBuilder::MakeUidsChangedBuilder(true, 0);
      send_message(label, false, std::move(response));
    } break;

    default: {
      log::error("{}: Unknown event registered. Event ID={}", address_, pkt->GetEventRegistered());
      auto response = RejectBuilder::MakeBuilder((CommandPdu)pkt->GetCommandPdu(),
...
void Device::HandleVolumeChanged(uint8_t label,
...
void Device::PlaybackPosNotificationResponse(uint8_t label, bool interim, PlayStatus status) {
  log::verbose("");
  ...
  auto response = RegisterNotificationResponseBuilder::MakePlaybackPositionBuilder(interim,
                                                                                   status.position);
  send_message_cb_.Run(label, false, std::move(response));
  ...
  switch (pkt->GetOpcode()) {
    ...
    case Opcode::PASS_THROUGH: {
      ...
      if (!pass_through_packet->IsValid()) {
        log::warn("{}: Request packet is not valid", address_);
        auto response =
                RejectBuilder::MakeBuilder(static_cast<CommandPdu>(0), Status::INVALID_COMMAND);
        send_message(label, false, std::move(response));
        return;
      }
...
void Device::HandlePlayItem(uint8_t label, std::shared_ptr<PlayItemRequest> pkt) {
  if (!pkt->IsValid()) {
    log::warn("{}: Request packet is not valid", address_);
    auto response = RejectBuilder::MakeBuilder(pkt->GetCommandPdu(), Status::INVALID_PARAMETER);
    send_message(label, false, std::move(response));
  ...
  if (media_id == "") {
    ...
    auto response = RejectBuilder::MakeBuilder(CommandPdu::PLAY_ITEM, Status::DOES_NOT_EXIST);
    send_message(label, false, std::move(response));
    return;
  }

  media_interface_->PlayItem(curr_browsed_player_id_, pkt->GetScope() == Scope::NOW_PLAYING,
                             media_id);
...
void Device::ListPlayerApplicationSettingValuesResponse(uint8_t label, PlayerAttribute attribute,
                                                        std::vector<uint8_t> values) {
  uint8_t number_of_values = values.size();
  log::verbose("attribute={}, number_of_values={}", attribute, number_of_values);
  ...
  auto response = ListPlayerApplicationSettingValuesResponseBuilder::MakeBuilder(std::move(values));
  send_message(label, false, std::move(response));
}

void Device::GetPlayerApplicationSettingValueResponse(uint8_t label,
                                                      std::vector<PlayerAttribute> attributes,
                                                      std::vector<uint8_t> values) {
  for (size_t i = 0; i < attributes.size(); i++) {
  ...
  auto response = GetCurrentPlayerApplicationSettingValueResponseBuilder::MakeBuilder(
          std::move(attributes), std::move(values));
  send_message(label, false, std::move(response));
}

void Device::SetPlayerApplicationSettingValueResponse(uint8_t label, CommandPdu pdu, bool success) {
  if (!success) {
  ...
  auto response = SetPlayerApplicationSettingValueResponseBuilder::MakeBuilder();
  send_message(label, false, std::move(response));
}

void Device::BrowseMessageReceived(uint8_t label, std::shared_ptr<BrowsePacket> pkt) {
  if (!pkt->IsValid()) {
    log::warn("{}: Request packet is not valid", address_);
  ...
  switch (pkt->GetPdu()) {
    case BrowsePdu::SET_BROWSED_PLAYER:
      HandleSetBrowsedPlayer(label, Packet::Specialize<SetBrowsedPlayerRequest>(pkt));
      break;
    case BrowsePdu::GET_FOLDER_ITEMS:
      HandleGetFolderItems(label, Packet::Specialize<GetFolderItemsRequest>(pkt));
      break;
    ...
    case BrowsePdu::GET_ITEM_ATTRIBUTES:
      HandleGetItemAttributes(label, Packet::Specialize<GetItemAttributesRequest>(pkt));
      break;
    case BrowsePdu::GET_TOTAL_NUMBER_OF_ITEMS:
      HandleGetTotalNumberOfItems(label, Packet::Specialize<GetTotalNumberOfItemsRequest>(pkt));
      break;
    default:
...
void Device::HandleGetFolderItems(uint8_t label, std::shared_ptr<GetFolderItemsRequest> pkt) {
  if (!pkt->IsValid()) {
    ...
    auto response = GetFolderItemsResponseBuilder::MakePlayerListBuilder(Status::INVALID_PARAMETER,
                                                                         0x0000, browse_mtu_);
    send_message(label, true, std::move(response));
    return;
  }
  ...
  switch (pkt->GetScope()) {
    case Scope::MEDIA_PLAYER_LIST:
      media_interface_->GetMediaPlayerList(base::Bind(&Device::GetMediaPlayerListResponse,
                                                      weak_ptr_factory_.GetWeakPtr(), label, pkt));
      break;
    case Scope::VFS:
      media_interface_->GetFolderItems(
              curr_browsed_player_id_, CurrentFolder(),
              base::Bind(&Device::GetVFSListResponse, weak_ptr_factory_.GetWeakPtr(), label, pkt));
      break;
    case Scope::NOW_PLAYING:
      media_interface_->GetNowPlayingList(base::Bind(&Device::GetNowPlayingListResponse,
                                                     weak_ptr_factory_.GetWeakPtr(), label, pkt));
      break;
    default:
...
void Device::HandleGetTotalNumberOfItems(uint8_t label,
                                         std::shared_ptr<GetTotalNumberOfItemsRequest> pkt) {
  if (!pkt->IsValid()) {
    log::warn("{}: Request packet is not valid", address_);
    auto response =
            GetTotalNumberOfItemsResponseBuilder::MakeBuilder(Status::INVALID_PARAMETER, 0x0000, 0);
    send_message(label, true, std::move(response));
    return;
  }
  ...
  switch (pkt->GetScope()) {
    ...
    case Scope::VFS:
      media_interface_->GetFolderItems(curr_browsed_player_id_, CurrentFolder(),
...
void Device::HandleGetItemAttributes(uint8_t label, std::shared_ptr<GetItemAttributesRequest> pkt) {
  if (!pkt->IsValid()) {
    log::warn("{}: Request packet is not valid", address_);
    auto builder =
            GetItemAttributesResponseBuilder::MakeBuilder(Status::INVALID_PARAMETER, browse_mtu_);
    send_message(label, true, std::move(builder));
    return;
  }

  log::verbose("scope={} uid=0x{:x} uid counter=0x{:x}", pkt->GetScope(), pkt->GetUid(),
               pkt->GetUidCounter());
  if (pkt->GetUidCounter() != 0x0000) {  // For database unaware player, use 0
    log::warn("{}: UidCounter is invalid", address_);
    auto builder = GetItemAttributesResponseBuilder::MakeBuilder(Status::UIDS_CHANGED, browse_mtu_);
    send_message(label, true, std::move(builder));
    return;
  }
...
void Device::GetItemAttributesVFSResponse(uint8_t label,
                                          std::shared_ptr<GetItemAttributesRequest> pkt,
  ...
  if (media_id == "") {
    ...
    auto builder =
            GetItemAttributesResponseBuilder::MakeBuilder(Status::DOES_NOT_EXIST, browse_mtu_);
    send_message(label, true, std::move(builder));
    return;
  }
    ...
    if (attributes_requested.size() != 0) {
      for (const auto& attribute : attributes_requested) {
        if (item_requested.song.attributes.find(attribute) !=
          ...
          builder->AddAttributeEntry(*item_requested.song.attributes.find(attribute));
        }
      }
    } else {
      // If zero attributes were requested, that means all attributes were
...
void Device::GetVFSListResponse(uint8_t label, std::shared_ptr<GetFolderItemsRequest> pkt,
                                std::vector<ListItem> items) {
  log::verbose("start_item={} end_item={}", pkt->GetStartItem(), pkt->GetEndItem());

  // The builder will automatically correct the status if there are zero items
  auto builder =
          GetFolderItemsResponseBuilder::MakeVFSBuilder(Status::NO_ERROR, 0x0000, browse_mtu_);
  ...
  for (auto i = pkt->GetStartItem(); i <= pkt->GetEndItem() && i < items.size(); i++) {
    if (items[i].type == ListItem::FOLDER) {
      ...
      if (pkt->GetNumAttributes() == 0x00) {  // All attributes requested
        ...
        song_item.attributes_ = filter_attributes_requested(song, pkt->GetAttributesRequested());
      }

      // If we fail to add a song, don't accidentally add one later that might
      // fit.
      if (!builder->AddSong(song_item)) {
        break;
      }
    }
  }
...
void Device::GetNowPlayingListResponse(uint8_t label, std::shared_ptr<GetFolderItemsRequest> pkt,
                                       std::string /* unused curr_song_id */,
                                       std::vector<SongInfo> song_list) {
  log::verbose("");
  ...
  for (size_t i = pkt->GetStartItem(); i <= pkt->GetEndItem() && i < song_list.size(); i++) {
    ...
    if (pkt->GetNumAttributes() == 0x00) {
      ...
      item.attributes_ = filter_attributes_requested(song, pkt->GetAttributesRequested());
    }

    // If we fail to add a song, don't accidentally add one later that might
    // fit.
    if (!builder->AddSong(item)) {
      break;
    }
  }
...
void Device::HandleSetBrowsedPlayer(uint8_t label, std::shared_ptr<SetBrowsedPlayerRequest> pkt) {
  if (!pkt->IsValid()) {
    log::warn("{}: Request packet is not valid", address_);
...
void Device::SendMediaUpdate(bool metadata, bool play_status, bool queue) {
  ...
  log::assert_that(media_interface_ != nullptr, "assert failed: media_interface_ != nullptr");
  log::verbose("Metadata={} : play_status= {} : queue={} : is_silence={}", metadata, play_status,
               queue, is_silence);
...
void Device::PlayerSettingChangedNotificationResponse(uint8_t label, bool interim,
                                                      std::vector<PlayerAttribute> attributes,
                                                      std::vector<uint8_t> values) {
...
void Device::HandleNowPlayingNotificationResponse(uint8_t label, bool interim,
  ...
  auto response = RegisterNotificationResponseBuilder::MakeNowPlayingBuilder(interim);
...
void Device::HandleAvailablePlayerUpdate() {
  ...
  auto response = RegisterNotificationResponseBuilder::MakeAvailablePlayersBuilder(false);
  send_message_cb_.Run(avail_players_changed_.second, false, std::move(response));
...
static std::string volumeToStr(int8_t volume) {
  if (volume == VOL_NOT_SUPPORTED) {
    return "Absolute Volume not supported";
  }
...
std::ostream& operator<<(std::ostream& out, const Device& d) {
  ...
  out << "    Current Browsed Player ID: " << d.curr_browsed_player_id_ << std::endl;
  ...
  if (d.player_setting_changed_.first) {
    out << "        Player Setting Changed\n";
  }
  if (d.now_playing_changed_.first) {
    out << "        Now Playing\n";
  }
  if (d.addr_player_changed_.first) {
    out << "        Addressed Player\n";
  }
  if (d.avail_players_changed_.first) {
    out << "        Available Players\n";
  }
