...
Sender::Sender(os::Handler* handler, ILink* link, Scheduler* scheduler,
               std::shared_ptr<ChannelImpl> channel)
    : handler_(handler),
      link_(link),
      queue_end_(channel->GetQueueDownEnd()),
      scheduler_(scheduler),
      channel_id_(channel->GetCid()),
      remote_channel_id_(channel->GetRemoteCid()),
      data_controller_(std::make_unique<BasicModeDataController>(
              channel_id_, remote_channel_id_, queue_end_, handler_, scheduler_)) {
...
Sender::Sender(os::Handler* handler, ILink* link, Scheduler* scheduler,
  ...
  if (mode == ChannelMode::BASIC) {
    data_controller_ = std::make_unique<BasicModeDataController>(channel_id_, remote_channel_id_,
                                                                 queue_end_, handler_, scheduler_);
  } else if (mode == ChannelMode::ERTM) {
    data_controller_ = std::make_unique<ErtmController>(link_, channel_id_, remote_channel_id_,
                                                        queue_end_, handler_, scheduler_);
  } else if (mode == ChannelMode::LE_CREDIT_BASED) {
    data_controller_ = std::make_unique<LeCreditBasedDataController>(
            link_, channel_id_, remote_channel_id_, queue_end_, handler_, scheduler_);
  }
  try_register_dequeue();
}
...
DataController* Sender::GetDataController() { return data_controller_.get(); }

void Sender::try_register_dequeue() {
  if (is_dequeue_registered_.exchange(true)) {
    return;
  }
  queue_end_->RegisterDequeue(handler_,
                              common::Bind(&Sender::dequeue_callback, common::Unretained(this)));
}
...
void Sender::dequeue_callback() {
  ...
  handler_->Post(common::BindOnce(&DataController::OnSdu,
                                  common::Unretained(data_controller_.get()), std::move(packet)));
  if (is_dequeue_registered_.exchange(false)) {
    queue_end_->UnregisterDequeue();
  }
...
void Sender::UpdateClassicConfiguration(classic::internal::ChannelConfigurationState config) {
  ...
  if (mode == RetransmissionAndFlowControlModeOption::ENHANCED_RETRANSMISSION) {
    data_controller_ = std::make_unique<ErtmController>(link_, channel_id_, remote_channel_id_,
                                                        queue_end_, handler_, scheduler_);
    RetransmissionAndFlowControlConfigurationOption option =
            config.local_retransmission_and_flow_control_;
