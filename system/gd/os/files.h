...
// Implement ability to read a whole file from |path| into a C++ string, return std::nullopt on
// failure
//
// Do not use this with large files
std::optional<std::string> ReadSmallFile(const std::string& path);

// Implement ability to safely write to a file. This function is needed because of deficiencies in
// existing C++ file libraries, namely:
...
// Print error log when file is failed to be removed, hence user should make sure file exists before
// calling this Return true on success, false on failure (e.g. file not exist, failed to remove,
// etc)
