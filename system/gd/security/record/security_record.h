...
public:
  ...
  bool IsPaired() const { return IsClassicLinkKeyValid(); }
  ...
  void CancelPairing() { pairing_ = false; }
  ...
  std::optional<hci::AddressWithType> GetPseudoAddress() { return pseudo_address_; }

  void SetAuthenticated(bool is_authenticated) { this->is_authenticated_ = is_authenticated; }

  bool IsAuthenticated() { return this->is_authenticated_; }

  void SetRequiresMitmProtection(bool requires_mitm_protection) {
    this->requires_mitm_protection_ = requires_mitm_protection;
  }

  bool RequiresMitmProtection() { return this->requires_mitm_protection_; }

  void SetIsEncryptionRequired(bool is_encryption_required) {
    this->is_encryption_required_ = is_encryption_required;
  }

  bool IsEncryptionRequired() { return this->is_encryption_required_; }

  bool IsClassicLinkKeyValid() const {
    return !std::all_of(link_key_.begin(), link_key_.end(), [](uint8_t b) { return b == 0; });
  }

  void SetIsTemporary(bool is_temp) { this->is_temporary_ = is_temp; }

  bool IsTemporary() { return this->is_temporary_; }
