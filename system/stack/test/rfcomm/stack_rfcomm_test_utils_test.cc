...
TEST(RfcommTestPacketGeneratorTest, TestGenerateSabmChannel0Packet) {
  ...
  std::vector<uint8_t> l2cap_packet = CreateL2capDataPacket(0x005c, rfcomm_packet);
  std::vector<uint8_t> acl_packet = CreateAclPacket(0x0008, 0b10, 0b00, l2cap_packet);
...
TEST(RfcommTestPacketGeneratorTest, TestGenerateUaPacket) {
  ...
  std::vector<uint8_t> rfcomm_packet = CreateRfcommPacket(address_field, control_field, -1, {});
  std::vector<uint8_t> l2cap_packet = CreateL2capDataPacket(0x1700, rfcomm_packet);
  std::vector<uint8_t> acl_packet = CreateAclPacket(0x0008, 0b10, 0b00, l2cap_packet);
...
const uint8_t kIncomingUihPnSetChannelTo3[] = {0x08, 0x20, 0x12, 0x00, 0x0e, 0x00, 0x5c, 0x00,
                                               0x03, 0xef, 0x15, 0x83, 0x11, 0x06, 0xf0, 0x00,
                                               0x00, 0x74, 0x03, 0x00, 0x01, 0x70};
...
TEST(RfcommTestPacketGeneratorTest, TestGenerateUihPnSetChannel3Packet) {
  ...
  std::vector<uint8_t> mcc_pn_data = CreateMccPnFrame(new_dlci, 0x0, 0xF, 0, 0, 884, 0, 1);
  std::vector<uint8_t> mcc_payload = CreateMultiplexerControlFrame(0x20, true, mcc_pn_data);
...
TEST(RfcommTestPacketGeneratorTest, TestQuickGenerateUihPnSetChannel3Packet) {
  EXPECT_THAT(CreateQuickPnPacket(true, GetDlci(false, 3), true, 884, 0xF, 0, 1, 0x005c, 0x0008),
...
const uint8_t kIncomingUihMscCmdFrame[] = {0x08, 0x20, 0x0c, 0x00, 0x08, 0x00, 0x5c, 0x00,
                                           0x03, 0xef, 0x09, 0xe3, 0x05, 0x1b, 0x8d, 0x70};
...
TEST(RfcommTestPacketGeneratorTest, TestGenerateUihMscCmdPacket) {
  ...
  uint8_t new_dlci = GetDlci(false, 3);
  EXPECT_EQ(new_dlci, 6);
  std::vector<uint8_t> mcc_msc_data = CreateMccMscFrame(new_dlci, false, true, true, false, true);
  std::vector<uint8_t> mcc_payload = CreateMultiplexerControlFrame(0x38, true, mcc_msc_data);
  std::vector<uint8_t> rfcomm_packet =
          CreateRfcommPacket(address_field, control_field, -1, mcc_payload);
  std::vector<uint8_t> l2cap_packet = CreateL2capDataPacket(0x005c, rfcomm_packet);
  std::vector<uint8_t> acl_packet = CreateAclPacket(0x0008, 0b10, 0b00, l2cap_packet);
...
TEST(RfcommTestPacketGeneratorTest, TestQuickGenerateUihMscCmdPacket) {
  EXPECT_THAT(CreateQuickMscPacket(true, GetDlci(false, 3), 0x005c, 0x0008, true, false, true, true,
                                   false, true),
              ElementsAreArray(kIncomingUihMscCmdFrame));
}

const uint8_t kIncomingUihMscResponseFrame[] = {0x08, 0x20, 0x0c, 0x00, 0x08, 0x00, 0x5c, 0x00,
...
TEST(RfcommTestPacketGeneratorTest, TestGenerateUihMscResponsePacket) {
  ...
  EXPECT_EQ(new_dlci, 6);
  std::vector<uint8_t> mcc_msc_data = CreateMccMscFrame(new_dlci, false, true, true, false, true);
  std::vector<uint8_t> mcc_payload = CreateMultiplexerControlFrame(0x38, false, mcc_msc_data);
  std::vector<uint8_t> rfcomm_packet =
          CreateRfcommPacket(address_field, control_field, -1, mcc_payload);
  std::vector<uint8_t> l2cap_packet = CreateL2capDataPacket(0x005c, rfcomm_packet);
  std::vector<uint8_t> acl_packet = CreateAclPacket(0x0008, 0b10, 0b00, l2cap_packet);
...
TEST(RfcommTestPacketGeneratorTest, TestQuickGenerateUihMscResponsePacket) {
  EXPECT_THAT(CreateQuickMscPacket(true, GetDlci(false, 3), 0x005c, 0x0008, false, false, true,
                                   true, false, true),
              ElementsAreArray(kIncomingUihMscResponseFrame));
}

const uint8_t kIncomingBrsfFrame[] = {0x08, 0x20, 0x15, 0x00, 0x11, 0x00, 0x5c, 0x00, 0x1b,
                                      0xff, 0x19, 0x06, 0x41, 0x54, 0x2b, 0x42, 0x52, 0x53,
                                      0x46, 0x3d, 0x39, 0x35, 0x39, 0x0d, 0x93};
...
TEST(RfcommTestPacketGeneratorTest, TestGenerateDataPacket) {
  ...
  std::vector<uint8_t> rfcomm_packet = CreateRfcommPacket(address_field, control_field, 6, data);
  std::vector<uint8_t> l2cap_packet = CreateL2capDataPacket(0x005c, rfcomm_packet);
  std::vector<uint8_t> acl_packet = CreateAclPacket(0x0008, 0b10, 0b00, l2cap_packet);
...
TEST(RfcommTestPacketGeneratorTest, TestQuickGenerateDataPacket) {
  EXPECT_THAT(CreateQuickDataPacket(GetDlci(false, 3), true, 0x005c, 0x0008, 6, "AT+BRSF=959\r"),
