...
static const tA2DP_APTX_HD_CIE a2dp_aptx_hd_source_caps = {
        ...
        (A2DP_APTX_HD_SAMPLERATE_44100 | A2DP_APTX_HD_SAMPLERATE_48000), /* sampleRate */
...
static tA2DP_STATUS A2DP_BuildInfoAptxHd(uint8_t media_type, const tA2DP_APTX_HD_CIE* p_ie,
                                         uint8_t* p_result) {
...
// status code.
static tA2DP_STATUS A2DP_ParseInfoAptxHd(tA2DP_APTX_HD_CIE* p_ie, const uint8_t* p_codec_info,
                                         bool is_capability) {
  uint8_t losc;
  uint8_t media_type;
  ...
  losc = *p_codec_info++;
  if (losc != A2DP_APTX_HD_CODEC_LEN) {
...
bool A2DP_VendorCodecTypeEqualsAptxHd(const uint8_t* p_codec_info_a,
  ...
  tA2DP_STATUS a2dp_status = A2DP_ParseInfoAptxHd(&aptx_hd_cie_a, p_codec_info_a, true);
...
int A2DP_VendorGetTrackSampleRateAptxHd(const uint8_t* p_codec_info) {
  ...
  // Check whether the codec info contains valid data
  tA2DP_STATUS a2dp_status = A2DP_ParseInfoAptxHd(&aptx_hd_cie, p_codec_info, false);
  if (a2dp_status != A2DP_SUCCESS) {
    log::error("cannot decode codec information: {}", a2dp_status);
    return -1;
  }
...
std::string A2DP_VendorCodecInfoStringAptxHd(const uint8_t* p_codec_info) {
  ...
  if (a2dp_status != A2DP_SUCCESS) {
    res << "A2DP_ParseInfoAptxHd fail: " << loghex(static_cast<uint8_t>(a2dp_status));
    return res.str();
  }
  ...
  AppendField(&field, (aptx_hd_cie.sampleRate & A2DP_APTX_HD_SAMPLERATE_44100), "44100");
  AppendField(&field, (aptx_hd_cie.sampleRate & A2DP_APTX_HD_SAMPLERATE_48000), "48000");
  res << "\tsamp_freq: " << field << " (" << loghex(aptx_hd_cie.sampleRate) << ")\n";

  // Channel mode
  field.clear();
  AppendField(&field, (aptx_hd_cie.channelMode == 0), "NONE");
  AppendField(&field, (aptx_hd_cie.channelMode & A2DP_APTX_HD_CHANNELS_MONO), "Mono");
  AppendField(&field, (aptx_hd_cie.channelMode & A2DP_APTX_HD_CHANNELS_STEREO), "Stereo");
...
const tA2DP_ENCODER_INTERFACE* A2DP_VendorGetEncoderInterfaceAptxHd(const uint8_t* p_codec_info) {
  if (!A2DP_IsVendorSourceCodecValidAptxHd(p_codec_info)) {
    return NULL;
  }
...
bool A2DP_VendorInitCodecConfigAptxHd(AvdtpSepConfig* p_cfg) {
  if (A2DP_BuildInfoAptxHd(AVDT_MEDIA_TYPE_AUDIO, &a2dp_aptx_hd_source_caps, p_cfg->codec_info) !=
      A2DP_SUCCESS) {
    return false;
  }
...
A2dpCodecConfigAptxHd::A2dpCodecConfigAptxHd(btav_a2dp_codec_priority_t codec_priority)
  ...
  codec_local_capability_.bits_per_sample = a2dp_aptx_hd_source_caps.bits_per_sample;
...
//
static bool select_best_sample_rate(uint8_t sampleRate, tA2DP_APTX_HD_CIE* p_result,
...
static bool select_audio_sample_rate(const btav_a2dp_codec_config_t* p_codec_audio_config,
                                     uint8_t sampleRate, tA2DP_APTX_HD_CIE* p_result,
                                     btav_a2dp_codec_config_t* p_codec_config) {
...
// Returns true if a selection was made, otherwise false.
//
static bool select_best_channel_mode(uint8_t channelMode, tA2DP_APTX_HD_CIE* p_result,
                                     btav_a2dp_codec_config_t* p_codec_config) {
...
static bool select_audio_channel_mode(const btav_a2dp_codec_config_t* p_codec_audio_config,
                                      uint8_t channelMode, tA2DP_APTX_HD_CIE* p_result,
                                      btav_a2dp_codec_config_t* p_codec_config) {
...
bool A2dpCodecConfigAptxHd::setCodecConfig(const uint8_t* p_peer_codec_info, bool is_capability,
  ...
  tA2DP_STATUS status = A2DP_ParseInfoAptxHd(&peer_info_cie, p_peer_codec_info, is_capability);
  if (status != A2DP_SUCCESS) {
  ...
  do {
    ...
    if (select_best_sample_rate(a2dp_aptx_hd_default_config.sampleRate & peer_info_cie.sampleRate,
  ...
  do {
    // Compute the selectable capability
    codec_selectable_capability_.bits_per_sample = a2dp_aptx_hd_source_caps.bits_per_sample;
  ...
  //
  channelMode = a2dp_aptx_hd_source_caps.channelMode & peer_info_cie.channelMode;
  ...
  result_config_cie.acl_sprint_reserved0 =
          a2dp_aptx_hd_source_caps.acl_sprint_reserved0 & peer_info_cie.acl_sprint_reserved0;
  result_config_cie.acl_sprint_reserved1 =
          a2dp_aptx_hd_source_caps.acl_sprint_reserved1 & peer_info_cie.acl_sprint_reserved1;
  result_config_cie.acl_sprint_reserved2 =
          a2dp_aptx_hd_source_caps.acl_sprint_reserved2 & peer_info_cie.acl_sprint_reserved2;
  result_config_cie.acl_sprint_reserved3 =
          a2dp_aptx_hd_source_caps.acl_sprint_reserved3 & peer_info_cie.acl_sprint_reserved3;

  if (A2DP_BuildInfoAptxHd(AVDT_MEDIA_TYPE_AUDIO, &result_config_cie, p_result_codec_config) !=
      A2DP_SUCCESS) {
    goto fail;
  }
  ...
  if (is_capability) {
    status =
            A2DP_BuildInfoAptxHd(AVDT_MEDIA_TYPE_AUDIO, &peer_info_cie, ota_codec_peer_capability_);
  } else {
    status = A2DP_BuildInfoAptxHd(AVDT_MEDIA_TYPE_AUDIO, &peer_info_cie, ota_codec_peer_config_);
  }
  log::assert_that(status == A2DP_SUCCESS, "assert failed: status == A2DP_SUCCESS");
  status = A2DP_BuildInfoAptxHd(AVDT_MEDIA_TYPE_AUDIO, &result_config_cie, ota_codec_config_);
  log::assert_that(status == A2DP_SUCCESS, "assert failed: status == A2DP_SUCCESS");
  return true;
...
bool A2dpCodecConfigAptxHd::setPeerCodecCapabilities(const uint8_t* p_peer_codec_capabilities) {
  ...
  tA2DP_STATUS status = A2DP_ParseInfoAptxHd(&peer_info_cie, p_peer_codec_capabilities, true);
  if (status != A2DP_SUCCESS) {
    log::error("can't parse peer's capabilities: error = {}", status);
    goto fail;
  }
