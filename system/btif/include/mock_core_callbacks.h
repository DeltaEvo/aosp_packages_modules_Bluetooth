...
struct MockCodecInterface : public CodecInterface {
  ...
  MOCK_METHOD((bool), decodePacket, (const uint8_t* i_buf, int16_t* o_buf, size_t out_len), ());
};
...
struct MockCoreInterface : public CoreInterface {
  MockCoreInterface()
      : CoreInterface(&mock_event_callbacks, &mock_config_interface, &mock_codec_msbcCodec,
                      &mock_codec_lc3Codec, &mock_HACK_profile_interface) {}

  MOCK_METHOD((void), onBluetoothEnabled, (), ());
  MOCK_METHOD((bt_status_t), toggleProfile, (tBTA_SERVICE_ID service_id, bool enable), ());
  MOCK_METHOD((void), removeDeviceFromProfiles, (const RawAddress& bd_addr), ());
  MOCK_METHOD((void), onLinkDown, (const RawAddress& bd_addr, tBT_TRANSPORT transport), ());
};
