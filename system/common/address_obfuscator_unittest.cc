...
static std::string kTestResult1(reinterpret_cast<const char*>(kTestResultRaw1.data()),
                                kTestResultRaw1.size());

constexpr AddressObfuscator::Octet32 kTestKey2 = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
...
static std::string kTestResult2_1(reinterpret_cast<const char*>(kTestResultRaw2_1.data()),
...
static std::string kTestResult2_2(reinterpret_cast<const char*>(kTestResultRaw2_2.data()),
...
static std::string kTestResult2_3(reinterpret_cast<const char*>(kTestResultRaw2_3.data()),
...
TEST(AddressObfuscatorTest, test_obfuscate_address_key2) {
  ...
  std::string result = AddressObfuscator::GetInstance()->Obfuscate(kTestData2_1);
  EXPECT_EQ(result.size(), AddressObfuscator::kOctet32Length);
...
TEST(AddressObfuscatorTest, test_obfuscate_address_key2_empty_adddress) {
  AddressObfuscator::GetInstance()->Initialize(kTestKey2);
  std::string result = AddressObfuscator::GetInstance()->Obfuscate(kTestData2_2);
  EXPECT_EQ(result.size(), AddressObfuscator::kOctet32Length);
...
TEST(AddressObfuscatorTest, test_obfuscate_address_key2_max_address) {
  AddressObfuscator::GetInstance()->Initialize(kTestKey2);
  std::string result = AddressObfuscator::GetInstance()->Obfuscate(kTestData2_3);
  EXPECT_EQ(result.size(), AddressObfuscator::kOctet32Length);
