...
public:
   ...
   * by the provider.
   ***/
  std::optional<CodecInfo const*> GetCodec(btav_a2dp_codec_index_t codec_index) const;

  /***
   * Find the source codec index by codec capabilities.
   ***/
  std::optional<btav_a2dp_codec_index_t> SourceCodecIndex(CodecId const& codec_id) const;
  std::optional<btav_a2dp_codec_index_t> SourceCodecIndex(uint32_t vendor_id,
                                                          uint16_t codec_id) const;
  std::optional<btav_a2dp_codec_index_t> SourceCodecIndex(uint8_t const* codec_info) const;

  /***
   * Find the sink codec index by codec capabilities.
   ***/
  std::optional<btav_a2dp_codec_index_t> SinkCodecIndex(CodecId const& codec_id) const;
  std::optional<btav_a2dp_codec_index_t> SinkCodecIndex(uint32_t vendor_id,
                                                        uint16_t codec_id) const;
  std::optional<btav_a2dp_codec_index_t> SinkCodecIndex(uint8_t const* codec_info) const;

  /***
   * Return the name of the codec with the assigned
   * input index.
   ***/
  std::optional<const char*> CodecIndexStr(btav_a2dp_codec_index_t codec_index) const;
...
private:
  std::unordered_map<btav_a2dp_codec_index_t, CodecInfo const*> assigned_codec_indexes;
};
